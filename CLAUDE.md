...
- **Claude.ai Pro/Max subscription is the only provider** (`src/lib/claude-auth.ts`). **No secret ever lives in client JavaScript or `localStorage`** — the diagram is untrusted (any collaborator can edit it) so an XSS must not be able to read a credential. Chat is proxied through `POST /api/claude/messages` server-side; `streamChatMessage` (`claude-client.ts`) just sends `{ model, system, messages, tools? }` and consumes the SSE. The client `ClaudeAuthConfig` is a **secret-free marker** (`{ provider: "subscription", expiresAt }`); don't add secret fields to it.
- Model selection is **decoupled** from auth: the user picks a logical model id (`MODELS`) stored under `super-mermaid-claude-model`, and `resolveModelId()` maps it to the Anthropic id **in the route** at request time. Don't put a `model` field into the auth config.
- **Tool use / editor writes**: `streamChatMessage` runs a tool loop (bounded by `MAX_TOOL_ITERATIONS`), parsing `tool_use` blocks from the stream and feeding `tool_result` blocks back. The only tool today is `update_editor`, defined in `src/hooks/use-chat-sync.ts`; its `onToolUse` handler calls `updateContent` (passed into `useChatSync` from `editor-client.tsx`) so the assistant rewrites the shared diagram instead of pasting code into chat. The tool — and the editor-aware variant of the system prompt — are only enabled when `onApplyDiagram` is supplied to the hook. The proxy forwards `tools` only when present.
- **Subscription OAuth is split client/server because Anthropic's OAuth endpoints send no CORS headers** (the browser cannot call them):
  ...
  - Tokens live **only in the `sm_claude_sub` httpOnly cookie** set by `src/lib/claude-oauth-server.ts`. The client `SubscriptionAuth` is just a `{ provider, expiresAt }` marker; disconnect calls `clearSubscriptionSession()`.
  - `POST /api/claude/messages` reads the cookie, refreshes the token when near expiry (re-setting the cookie via `Set-Cookie`), attaches the bearer + `anthropic-beta: oauth-2025-04-20`, and streams the SSE response back.
  ...
  - Footgun: tokens are stored base64-JSON in one cookie; a very large payload could exceed the ~4KB cookie limit and be silently dropped by the browser.
